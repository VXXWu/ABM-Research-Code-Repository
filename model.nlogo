...
patches-own [
  ...
  NCHProvided
  heavySprayCanceled
;  sameOwnerNCH ; list of nch cells with same owner who gave it nch benefit
...
;  participateCluster
  ...
  nchStrategy
;  riskTolerance
;  trustIndex ; list of vals with all other farmers
  carePatch
  careFarmer
  careOthers
...
;;  set heterogeneityIndex random-float 10
;  set carePatchMean random-float 1
;  set careFarmerMean random-float 1
;  set careOthersMean random-float 1
;  set initialExperience 1 + ( random 11 )
;  set nchPercent random-float 1
;  set memory 5 + (random 45)
;;  set percentageSD random-float 1
;  set percentageSD random-float 1
;
;  set riskToleranceMean (random 20) * 1000
;  set riskToleranceSD (random 30) * 10
;  set maxRiskIncrement (random 30) * 10
;  set trustFactor random-float 1
;  set careOtherStrategy False
...
;
;  populate-landscape-heterogeneity
;
;  reset-ticks
;end
...
;  set riskToleranceMean (random 20) * 1000
;  set riskToleranceMean random 20000
;;  set riskToleranceSD (random 30) * 10
;  set riskToleranceSD random 300
;;  set maxRiskIncrement (random 30) * 10
;  set maxRiskIncrement random 300
;  set trustFactor random-float 1
  set careOtherStrategy True
  set understandSpatial False

;  set riskToleranceMean 1
;  set maxRiskIncrement 0
...
;  promote-new-choice
  seed-new-choices
  initial-run
;  show (word "NCH Strategy")
;  ask farmers [ show nchStrategy ]
...
to setup
  ...
  if country != "None" [
    ...
    set careFarmerMean item 5 parameters
    set careOthersMean item 6 parameters
    set careOtherStrategy true
    set understandSpatial false
...
;  set listDeviation lput overallNCHBenefit listDeviation
;  set listHSCells lput overallDeviation listHSCells
;  set listNCHBenefit lput overallHSCells listNCHBenefit
...
;  set listPopularStrategy lput overallPopularStrategy listPopularStrategy
  ...
  ask farmers [
    ...
    ifelse careOtherStrategy [
      ...
      ;      set nchYieldMemory n-values 5 [ n-values 1 [initialExperience * 36] ]
    ...
    [
      ...
      ;      set nchYieldMemory n-values 5 [ n-values 1 [initialExperience * 9] ]
...
;    set trustIndex [random-normal trustIndexMean percentageSD] of farmers
;    set trustIndex map [ ? -> ifelse-value (? < 0) [0] [?] ] trustIndex
  ]
...
;  set minPlotSize 4
;
;  let savedOutput outputData
;  let savedLoops loops
;  let savedSideLength sideLength
;  let savedNumFarms numFarms
;
;  set outputData savedOutput
;  set loops savedLoops
;  set sideLength savedSideLength
;  set numFarms savedNumFarms
;
;  set overallFarmerScore 0
;  set overallNetProduction 0
;  set overallNCHCells 0
...
;
;  ; Score/total patches
;  ; Net production/total patches
;  ; Subsidy received/total patches
;  ; NCH strategy/total farmers
;  ; Total NCH benefit received
;  ; Average deviation from mean score
;
;  ; moved to button
;;  set outputData n-values 8 [ [] ]
;;  set loops 0
;;
;;  set numFarms minNumFarms
;;  set sideLength minSideLength
;
;  ;; initialize globals
;;  set numFarms 4
;;  set choiceMeans (list choice0Mean choice1Mean) ;; one mean for each choice!
;;  set choiceSDs (list choice0SD choice1SD) ;; one SD for each choice!
;  ; blue green yellow red (base, NCH, LS, HS)
;  set choiceColors [95 65 45 15] ;; one color for each choice!
;  set-default-shape borders "line"
;  set-default-shape farmers "person farmer"
...
;  set fitness 0
;  set iterations 0
;  set totalStrategySwitches 0
;  set totalNCHSwitches 0
;  set totalNetProduction 0
;  set totalScore 0
;
;  seed-new-choices
...
;      ; 1, 2, 3, or 4 expected nch users next round (with 5 arrays since index 0 is the first array)
;      ;      set nchYieldMemory n-values 5 [ n-values 1 [initialExperience * 36] ]
;      set nchYieldMemory n-values 1 [initialExperience * 36]
;      set hsYieldMemory n-values 1 [initialExperience * 36]
;      set maxYieldMemory n-values 1 [initialExperience * 36]
;    ]
;    [
;      ; 1, 2, 3, or 4 expected nch users next round
;      ;      set nchYieldMemory n-values 5 [ n-values 1 [initialExperience * 9] ]
;      set nchYieldMemory n-values 1 [initialExperience * 9]
;      set hsYieldMemory n-values 1 [initialExperience * 9]
;      set maxYieldMemory n-values 1 [initialExperience * 9]
;    ]
;
;    ;reset mutable trust
;    set trustIndex [random-normal trustIndexMean percentageSD] of farmers
...
;let savedOutput outputData
;let savedLoops loops
;let s_overallFarmerScore overallFarmerScore ; Score/total patches
;let s_overallNetProduction overallNetProduction ; Net production/total patches
;let s_overallNCHCells overallNCHCells ; Subsidy received/total patches
;let s_overallNCHStrategy overallNCHStrategy ; NCH strategy/total farmers
;let s_overallNCHBenefit overallNCHBenefit; Total NCH benefit received
;let s_overallDeviation overallDeviation; Average deviation from mean score
;let s_overallHSCells overallHSCells
;
;setup
;
;set outputData savedOutput
;set loops savedLoops
;set overallFarmerScore s_overallFarmerScore; Score/total patches
;set overallNetProduction s_overallNetProduction; Net production/total patches
;set overallNCHCells s_overallNCHCells; Subsidy received/total patches
;set overallNCHStrategy s_overallNCHStrategy; NCH strategy/total farmers
;set overallNCHBenefit s_overallNCHBenefit; Total NCH benefit received
;set overallDeviation s_overallDeviation; Average deviation from mean score
;set overallHSCells s_overallHSCells
;
...
;
;    if nchStrategy = 1 [
;      ;      set nchYieldMemory update-sublist numNCH nchYieldMemory tempScore
;      ;      set nchYieldMemory replace-item numNCH nchYieldMemory trim-list (item numNCH nchYieldMemory) memory
;      set nchYieldMemory lput tempScore nchYieldMemory
;      set nchYieldMemory trim-list nchYieldMemory memory
;
;;      let tempHSYieldMemory hsYieldMemory
;
;
;;      ask link-neighbors [
;;        ifelse nchStrategy = 1 [
;;          let currTrust [trustIndex] of link-with farmer currentFarmer
;;;          set currTrust currTrust * (1 / trustFactor)
;;          ask link-with farmer currentFarmer [set trustIndex currTrust]
;;        ]
;;        [
;;          let currTrust [trustIndex] of link-with farmer currentFarmer
;;;          set currTrust currTrust * trustFactor
;;          ask link-with farmer currentFarmer [set trustIndex currTrust]
;;        ]
;;      ]
;
;      ; trust with other NCH farmers changes according to the difference between their current score and their "imagined" memory of scores from non-NCH
;;      let tempTrustIndex trustIndex
;;      let tempHSYieldMemory hsYieldMemory
;;      ask farmers with [nchStrategy = 1 and who != currentFarmer] [
;;        let currTrustDiff 1 - item who tempTrustIndex
;;        ifelse tempScore > 0 [
;;          set currTrustDiff currTrustDiff * (mean tempHSYieldMemory) / tempScore
;;        ]
;;        [ set currTrustDiff 1 ]
;;        set tempTrustIndex replace-item who tempTrustIndex (1 - currTrustDiff)
;;      ]
;
;;      ask farmers with [nchStrategy = 0] [
;;        let currTrust item who tempTrustIndex
;;        set currTrust currTrust * trustDecay
;;        set tempTrustIndex replace-item who tempTrustIndex currTrust
;;      ]
;;      set trustIndex tempTrustIndex
;    ]
;
;    if nchStrategy = 0 [
...
;;      ]
;
;      ; trust with other HS farmers as they do well
;;      let tempTrustIndex trustIndex
;;      let tempNCHYieldMemory nchYieldMemory
;;      ask farmers with [nchStrategy = 0 and who != currentFarmer] [
;;        let currTrustDiff 1 - item who tempTrustIndex
;;        ifelse tempScore > 0 [
;;          set currTrustDiff currTrustDiff * (mean tempNCHYieldMemory) / tempScore
;;        ]
;;        [ set currTrustDiff 1 ]
;;        set tempTrustIndex replace-item who tempTrustIndex (1 - currTrustDiff)
;;      ]
...
;    ]
;
;    ; trust changes according to change in score if they switched
;;    if ticks > 1 [
;;      if switchedGroup = 1 [
;;        let prevMembers item currCluster clusterOptionFarmers
;;        let otherMemory 0
;;        ifelse nchStrategy = 1 [
;;          set otherMemory (item (length hsYieldMemory - 1) hsYieldMemory) ]
;;        [set otherMemory (item (length nchYieldMemory - 1) nchYieldMemory) ]
;;        let const (farmerScore / otherMemory)
;;
;;        foreach prevMembers [ tempFarmer ->
;;          if tempFarmer != currentFarmer [
;;            ask link currentFarmer tempFarmer [
;;              set trustIndex trustIndex * const
;;            ]
;;          ]
;;        ]
;;      ]
...
;
;    ask farmer owner [
;      set currentPatchCount (currentPatchCount + 1)
;      ;;estimate certainty equivalents
;      let currentEstimates []
;;      let currRisk (item (currentPatchCount - 1) riskProfile)
;      let currDecay decayRate
;
;      foreach currExperiencePatch [ ?1 ->
;        ;        let curr ?1 * carePatch
;;        let currentCertEq certEq ?1 currRisk currDecay
;        let currentCertEq certEq ?1 carePatch currDecay
;;        show (word "patch level" (currentCertEq))
;
...
;
;      let ind 0
;      ;      set currRisk random-normal riskToleranceMean riskToleranceSD
;      foreach currExperienceFarmer [ ?1 ->
;        ;        let curr ?1 * careFarmer
;;        let currentCertEq certEq ?1 currRisk careFarmer
;        let currentCertEq certEq ?1 careFarmer currDecay
;
;        let oldValue item ind currentEstimates
;;        show (word "farmer level" (currentCertEq))
;        let newValue oldValue + (currentCertEq)
;        set currentEstimates replace-item ind currentEstimates newValue
;        ;        sxhow (word ind " " newValue)
;        set ind ind + 1
;      ]
;      ;      sxhow (word "others level")
;
;      if careOthersBool = 1 [
;        set ind 0
;        ;        set currRisk random-normal riskToleranceMean riskToleranceSD
;        foreach currExperienceOthers [ ?1 ->
;          ;          let curr ?1 * careOthers
;;          let currentCertEq certEq ?1 currRisk careOthers
;          let currentCertEq certEq ?1 careOthers currDecay
;
;          let oldValue item ind currentEstimates
;          ;          sxhow (word "others level" (currentCertEq / 27))
;          let newValue oldValue + (currentCertEq)
;;          show (word "others level" (currentCertEq / scaleOthers))
;
;;          set newValue newValue * (1 + random-float 0.2)
;          set currentEstimates replace-item ind currentEstimates newValue
;          ;          sxhow (word ind " " newValue)
;
;;          show (word currRisk " " newValue)
;
;          set ind ind + 1
;        ]
;      ]
;      if nchStrategy = 1 [
;        set currentEstimates (replace-item 3 currentEstimates 0)
;        ;        sxhow (word "currentEstimates " currentEstimates)
;      ]
;
;      ;      let currentEstimatesMod []
;;      set ind 0
;;      foreach currentEstimates [ ?1 ->
;;        ;        let temp (random-normal 1 riskToleranceSD)
;;        ;        let newValue ?1 * temp
;;        ;        if ind = 1 [
;;        ;          set newValue newValue * 1.5
;;        ;        ]
;;        set currentEstimates replace-item ind currentEstimates ?1
;;
;;        set ind ind + 1
;;      ]
;
...
;      ;
;      ;      ;; find best choice and switch to it
;      ;      let maxEstimate [item 0 sortedEstimates]
;      ;      set ind 1
;      ;      while item ind sortedEstimates = item 0 maxEstimate [
;      ;        set maxEstimate lput (item ind sortedEstimates) maxEstimate
;      ;      ]
;      let maxEstimate item 0 sortedEstimates
...
;      ;      set numChoice1 (numChoice1 + [choice] of patch currentX currentY)
;    ]
;  ]
;
;  ask farmers [
...
;  let temp 0
;  ask patches [
;    if choice = 1 [set temp temp + 1 ]
;  ]
;  set overallNCHCells overallNCHCells + (temp / totalPatches) / validationSplits
;
;  set temp 0
;;  set overallNCHStrategy overallNCHStrategy + ( (count farmers with [nchStrategy = 1]) / numFarms ) ; NCH strategy/total farmers
;  ask farmers [
;    if nchStrategy = 1 [set temp temp + 1]
;  ]
;
;  set overallNCHStrategy overallNCHStrategy + (temp / numFarms) / validationSplits
;;  sxhow (word overallNCHStrategy " " (temp / numFarms) )
;
;;  sxhow (word overallFarmerScore " " overallNCHCells " " overallNCHStrategy)
;  set temp 0
;;  set overallNCHBenefit overallNCHBenefit + ( (sum [receivedNCHBonus] of patches) / totalPatches ); Total NCH benefit received
...
;      set overallStrategySwitches overallStrategySwitches + 1 / validationSplits
;      if nchStrategy = 1 [
;        set overallNCHSwitches overallNCHSwitches + 1 / validationSplits
;      ]
;    ]
;  ]
;
...
;
;;  set reportFarmerScore 0
;;  set reportNetProduction 0
;;  set reportNCHCells 0
;;  set reportNCHStrategy 0
;;  set reportNCHBenefit 0
;;  set reportDeviation 0
;;  set reportHSCells 0
;
;  tick
;end
...
;      let currDecay decayRate
      ...
      foreach currExperiencePatch [ ?1 ->
        ;        let curr ?1 * carePatch
;        let currentCertEq certEq ?1 currRisk currDecay
;        let currentCertEq certEq ?1 currRisk carePatch
;        show (word "patch level" (currentCertEq))
...
;        let currentCertEq certEq ?1 careFarmer currDecay
;        let currentCertEq certEq ?1 currRisk careFarmer
        let currentCertEq (mean ?1) * careFarmer
...
;          let currentCertEq certEq ?1 careOthers currDecay
;          let currentCertEq certEq ?1 currRisk careOthers
        let currentCertEq (mean ?1) * careOthers
...
;          if newValue = 0 and currRisk > 10 ^ 20 [
;            set sizeStdDev 0
;            set endRun true
;            stop
;          ]
...
;  set overallNCHBenefit overallNCHBenefit + ( (sum [receivedNCHBonus] of patches) / totalPatches ); Total NCH benefit received
  ask patches [set temp temp + receivedNCHBonus]
;  ask patches [set temp temp + providedNCHBonus]
  let nchPatches count patches with [choice = 1]
  if nchPatches > 0 [
    set temp temp / (count patches with [choice = 1] )
  ]
...
;      if choice = 3 [ set overallHS_NCH overallHS_NCH + 1 / totalPatches / validationSplits]
      if choice = 1 [
  ...
  ask farmers with [nchStrategy = 0] [
    let currentFarmer who
    ask patches with [owner = currentFarmer] [
;      if choice = 3 [ set overallHS_NonNCH overallHS_NonNCH + 1 / totalPatches / validationSplits]
      if choice = 1 [
...
;to go
;  if (ticks mod validationSplits) = 0 [
;    if subsidy != -1 [
;       reset-model ]
;    set subsidy subsidy + 1
;  ]
...
;  farmers-switch
;
;  ; calculations pre farmers switch
;  let tempNCHBenefit 0
;  let tempNCHCells 0
;  ask farmers with [nchStrategy = 1] [
;    let currentFarmer who
...
;to populate-landscape-heterogeneity
;  let prevGrowthWeight 0.00001
...
;;      set growthWeight max ( list (random-normal 1 ( heterogeneityIndex ) ) 0 )
;;      set growthWeight random-normal 1 ( heterogeneityIndex )
;;      set minWeight min ( list minWeight growthWeight)
;
;;      set filled False
;;      ifelse length sizeList < numFarms / 2 [
;;        set setSize round ( random-normal avgPatches ( heterogeneityIndex * avgPatches ) )
;;        set setSize max (list setSize minPlotSize)
;;        set sizeList lput setSize sizeList
;;      ]
;;      [
;;        set setSize 2 * avgPatches - (item ind sizeList)
;;        set setSize max (list setSize minPlotSize)
;;        set ind ind + 1
;;      ]
;  ]
...
;
;;    if minWeight < 0 [
;;      ask farmers [set growthWeight growthWeight + abs( minWeight ) + 1 ]
;;    ]
;;    ask farmers [sxhow growthWeight]
;
;;    while [any? patches with [owner < 0]] [
;  while [any? farmers with [growthWeight > 0]] [
;    ask rnd:weighted-one-of farmers [ growthWeight ] [
;      let currentFarmer who
;      let borderPatches patch-set [neighbors4 with [owner < 0]] of ownPatches
;;        ask borderPatches [
;;          print (word "Patch at coordinates (" pxcor ", " pycor ")")
;;        ]
;
...
;  ask farmers [
;    let currentFarmer who
;    set sizeFarm count patches with [owner = currentFarmer]
...
;  set sizeRange maxSize - minSize
;end

to populate-landscape
;  set riskProfile (list max (list 1 max (list 100 (random-normal riskToleranceMean riskToleranceSD) )))
;    while [length riskProfile < sizeFarm] [
;     set riskProfile lput ((1 + (random-float maxRiskIncrement / 100)) * last riskProfile) riskProfile
;    ]
  ...
  ifelse uniformPlot [
    set numFarms 4
    create-farmers numFarms [
      ; owned by patches for now
      ;    set experience [ [] [] [] [] ]  ;; one nested list for each choice!
      set neighborList []
;      set edgePatches []
      set potentialClusters []
      ;setxy random-pxcor random-pycor
...
;    let prevGrowthWeight heterogeneityIndex
    create-farmers numFarms [
...
;    let totalFilledPatches 0

;    let under 0
    foreach sorted-farmers [?1 ->
      set growthList lput ([growthWeight] of ?1) growthList
;      let currGrowth [growthWeight] of ?1
;
;      let temp floor(currGrowth / sum growthList * totalPatches)
;      if temp < minPlotSize [
...
;      set ind ind + 1
;      if temp = minPlotSize [ set ind ind - 1 ]
    ]
...
;    if ind >= 1 [ set addBack ceiling(addBack / ind) ]
;    let cont True
;    foreach n-values (length sizeLimitList) [x -> x] [ i ->
;      if cont [
;        let temp item i sizeLimitList - addBack
;        set sizeLimitList replace-item i sizeLimitList temp
;        if i = ind [set cont False]
;      ]
;    ]

;    show growthList
;    show sizeLimitList
;    show sum sizeLimitList

    let ind 0
    foreach sorted-farmers [?1 ->
;      show [growthWeight] of ?1
      let currLimit item ind sizeLimitList
;      show currLimit

;      let currentFarmer 0
;      ask ?1 [
;        set currentFarmer who]
;
;      ask patches[
;        set unclaimed (neighbors4 with [owner < 0])
;        set sameOwner (neighbors4 with [owner = currentfarmer])
;      ]

      ask ?1 [
        let currentFarmer who  ; Get the who number of the farmer
;        show currentFarmer
...
;            any? neighbors with [ owner != [owner] of myself ]
          ]
...
;            ask one-of edgePatches [
;              let currOwner owner
;              let currSize [sizeFarm] of farmer currOwner
;              ; there are patches to give up
;              if currSize > 2 * minPlotSize [
;                set owner currentFarmer
;                set currX pxcor
;                set currY pycor
;                ask neighbors4 with [owner = currOwner] [ set owner currentFarmer]
...
;        set totalFilledPatches totalFilledPatches + 1
...
;          let surroundedPatches surrounded-patches currentFarmer
;          show surroundedPatches
          ifelse any? surroundedPatches [
;            let remain currLimit - sizeFarm
;            let cnt 0
            ask surroundedPatches [
;              if cnt < remain [
              set owner currentFarmer
;              sxhow (word "owner " owner)
...
;            set totalFilledPatches totalFilledPatches + ( count surroundedPatches )
          ]
          [
;            let borderPatches patch-set [neighbors4 with [owner < 0]] of ownPatches
...
;          show (word "sizeFarm " sizeFarm " currLimit " currLimit " neighborsLeft " neighborsLeft)
        ]

;        let surroundedPatches patches with [ owner < 0 and all? neighbors4 [ owner = currentFarmer] ]
;
;        while [any? surroundedPatches] [
;          ask surroundedPatches [
;            set owner currentFarmer
;
;            let newPatch self
...
;          ]
;          set sizeFarm sizeFarm + ( count surroundedPatches )
;          set surroundedPatches patches with [ owner < 0 and all? neighbors4 [ owner = currentFarmer] ]
;        ]
...
;    let totalFilledPatches count patches with [owner > -1]

;    while [totalFilledPatches < totalPatches ] [
    while [(count patches with [owner < 0] ) > 0 ] [
;    while [any? farmers with [growthWeight > 0]] [
;      let farmersList [self] of farmers with [neighborsLeft = true]
      let farmersList farmers with [neighborsLeft = true]
      let minFarmer min-one-of farmersList [sizeFarm]

;      ifelse empty? farmersList [
      ...
      [
;        ask one-of farmers with [neighborsLeft = true] [
        ask minFarmer [
          ;      ask rnd:weighted-one-of farmers [ growthWeight ] [
          let currentFarmer who
          let borderPatches patch-set []
...
;    foreach sorted-farmers [?1 ->
;      ask ?1 [
;        show item ind sizeLimitList
;        let currentFarmer who
;        show sizeFarm
;        show count patches with [owner = currentFarmer]
;        set ind ind + 1
;      ]
;    ]
  ]

    ;; allocate patches to farmers
;    while [any? patches with [owner < 0]] [
;      ask patches with [owner < 0] [
;        let tempList [owner] of neighbors4
;;        set tempList filter [value -> value >= 0] tempList
;
;        let ownerList []
;        foreach tempList [?1 ->
;          if ?1 >= 0 [
;            let tempFilled False
;            ask farmer ?1 [set tempFilled filled]
;            if tempFilled = False [
;              set ownerList lput ?1 ownerList
;            ]
;          ]
;        ]
;
;
;        if length ownerList > 0 [
;;          sxhow (word "ownerList " ownerList)
;          let patchCounts map [?1 ->
;            count patches with [owner = ?1]
;          ] ownerList
;;          sxhow (word "patchCounts " patchCounts)
;
;;          let lessList []
;          let finalOwner -1
;          let ownerPatches totalPatches
;          set ind 0
;          while [ind < length ownerList] [
;            let currOwner item ind ownerList
;;            let target [setSize] of farmer currOwner
;            let currPatches (count patches with [owner = currOwner] )
;;
;;            sxhow target
;
;;            if target > currPatches [
;            if [filled] of farmer currOwner = False  [
;;              set lessList lput currOwner lessList
;              if currPatches < ownerPatches [
;                set ownerPatches currPatches
;                set finalOwner currOwner
;              ]
;            ]
;;            sxhow (word "finalOwner " finalOwner)
;            set ind ind + 1
;          ]
;
;;          ifelse length lessList > 0 [
;          ifelse finalOwner >= 0 [
;            set owner finalOwner
;          ]
;          [
;            set owner item (random ( length ownerList ) ) ownerList
;          ]
;          let currOwner owner
;          let target [setSize] of farmer currOwner
;          let currPatches (count patches with [owner = currOwner] )
;
;          if target <= currPatches [
;            ask farmer owner [set filled True]
;          ]
;
;
;
;;          let chance heterogeneityIndex >= (random-float 1)
;;          ifelse chance [
;;            let sorted reverse sort patchCounts
;;            let maxPatches item 0 sorted
;;            let selectedInd position maxPatches patchCounts
;;            set owner ( item selectedInd ownerList)
;;;            set owner [owner] of one-of neighbors4
;;          ]
;;          [
;;            let sorted sort patchCounts
;;            let maxPatches item 0 sorted
;;            let selectedInd position maxPatches patchCounts
;;            set owner ( item selectedInd ownerList)
;;          ]
...
;    ]
  ...
  ask patches [
;    let neighborOwners [owner] of neighbors
;    set neighborOwners remove-duplicates neighborOwners
;    show owner
...
;    ask farmer currentOwner [
;      if length diffOwners > 0 [set potentialClusters lput diffOwners potentialClusters]
;    ]

;    let myX pxcor
;    let myY pycor
;    if edge = true [
;      ask farmer currentOwner [set edgePatches lput list (myX) (myY) edgePatches]
;    ]

;    let temp false
;    ask farmer currentOwner [
;      if length edgePatches > 0 and position ?1 edgePatches != false [
;        set temp true
;      ]
;    ]
;    if temp = false [
;      ask farmer currentOwner [set edgePatches lput ?1 edgePatches]
;    ]

    ; 4 types of cells
;    set experiencePatch n-values 4 [n-values 1 [initialExperience]]
;    set experienceFarmer n-values 4 [n-values 1 [initialExperience * ( [scaleFarmer] of farmer owner )]]
;    set experienceOthers n-values 4 [n-values 1 [initialExperience * ( [scaleOthers] of farmer owner )]]

;    let lowestTrust 1.1
;    let currentFarmer owner
;    let trustList [trustIndex] of one-of farmers with [who = currentFarmer]
;;    sxhow trustList
;
;    foreach neighborOwners [
;      currentOwner ->
;;      sxhow currentOwner
;;      sxhow item currentOwner trustList
;      if item currentOwner trustList < lowestTrust [
;        set experienceCategory currentOwner
;;        sxhow currentOwner
;      ]
;    ]
;    sxhow (word owner ", " experienceCategory)
  ]
...
;  ask patches [ set experience [[] [] [] []] ]
  ...
  ask farmers [
    ...
    ; riskProfile
    ; make it a single value for each famer
;    set riskProfile (list max (list 1 max (list 1 (random-normal riskToleranceMean percentageSD * 1000) )))
;;    set riskProfile (list riskToleranceMean)
;    while [length riskProfile < sizeFarm] [
;     set riskProfile lput ((1 + (random-float maxRiskIncrement)) * last riskProfile) riskProfile
;    ]
;    set decayRate random-normal decayRateMean percentageSD
...
;    set trustIndex [random-normal trustIndexMean percentageSD] of neighborList
;    set trustIndex map [ ? -> ifelse-value (? < 0) [0] [?] ] trustIndex
;    set trustIndex replace-item currentFarmer trustIndex 1000 ;ensure patch always falls into neighbor's experience category if available
;    sxhow trustIndex

    set carePatch random-normal carePatchMean percentageSD
    set carePatch max(list carePatch 0)
    set careFarmer random-normal careFarmerMean percentageSD
    set careFarmer max(list careFarmer 0)
    set careOthers random-normal careOthersMean percentageSD
    set careOthers max(list careOthers 0)

;      ; 1, 2, 3, or 4 expected nch users next round (with 5 arrays since index 0 is the first array)
;    set nchYieldMemory n-values 5 [ [] ]
;    set hsYieldMemory []
;    set maxYieldMemory []
...
to farmers-switch
;  ask farmers [;show nchYieldMemory
;    if empty? nchYieldMemory [
;      show (word "alert")
;      show trustIndexMean
;      show carePatchMean
;      show careFarmerMean
;      show careOthersMean
;      show initialExperience
;      show nchPercent
;      show memory
;      show percentageSD
;      show riskToleranceMean
;      show riskToleranceSD
;      show maxRiskIncrement
;      show trustFactor
;    ]
;  ]
;  ask farmers [;show hsYieldMemory
;    if empty? hsYieldMemory [
;      show (word "alert2")
;      show trustIndexMean
;      show carePatchMean
;      show careFarmerMean
...
;  set nchYieldMemory [initialExperience*9]
;      set hsYieldMemory [initialExperience*9]
;      set maxYieldMemory [initialExperience*9]

;  ask farmers with [who != currentFarmer] [
  ; 1 for nch, 0 for hs
  ...
  ask farmers [
    ...
    ; corresponding edge patch for each of the above clusters
;    let clustersPatchList []
...
;      sxhow otherFarmers
      ...
      foreach otherFarmers [ tempFarmer ->
        if tempFarmer != currentFarmer [
          ...
          ;        set currRank currRank + ([trustIndex] of link-with farmer tempFarmer farmer currentFarmer)
        ]
      ]
...
;        set it it + 1
    ...
    ; element of experimentation in proposed strategy
;    set nchPropose nchPropose + nchPropose * (random-float proposeRisk)
;    set hsPropose hsPropose + hsPropose * (random-float proposeRisk)

;    ifelse nchStrategy = 1
;    [set nchPropose nchPropose * switchDifficulty]
;    [set hsPropose hsPropose * switchDifficulty]
...
;        set currStrategies replace-item currentFarmer currStrategies 1
;        set currStrategyScores replace-item currentFarmer currStrategyScores nchPropose
      ]
      [
        ...
        set clusterOptionFarmers update-sublist wantedCluster clusterOptionFarmers currentFarmer
;        set currStrategies replace-item currentFarmer currStrategies 0
;        set currStrategyScores replace-item currentFarmer currStrategyScores hsPropose
      ]
    ]
    [
      ifelse nchPropose >= hsPropose [
...
;        sxhow (word "connect " selectedFarmer " " who)
        let trust [trustIndex] of link-with farmer selectedFarmer
        let adopt trust >= (random-float 1)
        if adopt [
          set nchStrategy selectedStrategy
          set switchedGroup 1
          ;      set totalStrategySwitches totalStrategySwitches + 1
        ]
      ]
    ]
    set ind ind + 1
  ...
  ; for farmers using opposite strategy, the chance they switch is their trust with the proposing farmer
;
;;    sxhow (word "numNCH " numNCH)
;;    sxhow (word "nchYieldMemory " nchYieldMemory)
;;    sxhow (word "expectedNCHYield " expectedNCHYield)
;;    sxhow (word "hsYieldMemory " hsYieldMemory)
;;    sxhow (word "expectedHSYield " expectedHSYield)
;;    sxhow (word "expectedMaxYield " expectedMaxYield)
;;    sxhow (word "expectedMaxYield " expectedMaxYield)


;    let expectedNumNCH numNCH
;    if nchStrategy = 0 [
;      set expectedNumNCH expectedNumNCH + 1
;    ]
;
;    ; expected yield if currFarmer is nch next round
;    let expectedNCHYield mean (item expectedNumNCH nchYieldMemory)
;    ; expected yield if landscape allows HS next round (at least 1 non-nch farmer)
;    let expectedHSYield mean hsYieldMemory
;    let expectedMaxYield max maxYieldMemory
;
;    let nchChance max (list 0 ( (expectedNCHYield - expectedHSYield) / expectedMaxYield) )
;    let stayChance strategyCert
;    ifelse nchStrategy = 1 [
;      let nchChance max (list 0 ( (expectedNCHYield - expectedHSYield) / expectedMaxYield) )
;      set stayChance stayChance + nchChance
;    ]
;    [
;      set stayChance stayChance + max (list 0 ( (expectedHSYield - expectedNCHYield) / expectedMaxYield) )
;    ]

;  set-current-plot "Number of NCH Users Each Round"
;  set numNCH count farmers with [nchStrategy = 1]
;  plotxy ticks numNCH
;
;  set-current-plot "Average Number of Label Switches"
;  let averageStrategySwitches totalStrategySwitches / (ticks + 1)
;;  sxhow (word "average StrategySwitches " averageStrategySwitches)
;  plotxy ticks averageStrategySwitches
;
;    set-current-plot "nchChance Distribution"
;    plotxy nchChance nchChance
;    plotxy stayChance stayChance

;    let compare random-float 1
;    ifelse nchChance >= compare [
;;    ifelse stayChance < compare [
;      ifelse nchStrategy = 0 [
;        set nchStrategy 1
;        set strategyCert random-normal strategyCertMean strategyCertSD
;        set strategyCert max(list strategyCert minStrategyCert)
;      ]
;      [ set strategyCert strategyCert + (1 - strategyCert) * strategyCertIncrement ]
;    ]
;    [
;      ifelse nchStrategy = 1 [
;        set nchStrategy 0
...
;  ask farmers with [nchStrategy = 1] [
;    let currentFarmer who
;    let nchFarmerScore farmerScore
;
;    let avgOtherScore (totalFarmerScore - nchFarmerScore) / 3
;    let switchOffChance (1 + (avgOtherScore - nchFarmerScore) / 100) * (1 - strategyCert)
;    let numNonNCH count farmers with [nchstrategy = 0]
;    set switchOffChance switchOffChance ^ (1 / (numNonNCH + 1))
;
;    let compare random-float 1
;    ifelse switchOffChance >= compare [
;      set nchStrategy 0
;      set strategyCert random-normal strategyCertMean strategyCertSD
;      set strategyCert max(list strategyCert minStrategyCert)
;    ]
;    [
;      ask farmers with [nchStrategy = 0 and switched = 0] [
;        let switchChance (1 + (nchFarmerScore - farmerScore) / 100) * (item currentFarmer trustIndex) * (1 - strategyCert)
;        set compare random-float 1
;        ifelse switchChance >= compare [
;          set nchStrategy 1
;          set strategyCert random-normal strategyCertMean strategyCertSD
;          set strategyCert max(list strategyCert minStrategyCert)
;        ]
...
;    set sameOwnerNCH []
  ...
  ; patches-own [owner choice currentHarvest currentYield currentSpending currentScore receivedNCHBonus providedNCHBonus NCHProviders]
...
;          set lostValue lostValue + nchBoost
;          if owner = currentOwner [
;            set lostValueSelf lostValueSelf + nchBoost
;          ]
        ]
      ]
      set receivedNCHBonus 0
...
;    set currentProvidedValue currentProvidedValue - lostValue
;    set currentProvidedValueSelf currentProvidedValue - lostValueSelf
  ] ;; end ask patches block benefits

;  ask patches with [choice = 1] [
;    let currentOwner owner
;    let providedValueSelf 0
;
;    foreach NCHProvided[ coords ->
;      ask patch item 0 coords item 1 coords [
;        if receivedNCHBonus > 0 and owner = currentOwner [
;          set providedValueSelf providedValueSelf + nchBoost
;        ]
;      ]
;    ]
;
;    set currentProvidedValueSelf currentProvidedValueSelf + providedValueSelf
;  ]

;  ask patches [
;    if choice = 1 [ set receivedNCHBonus 0 ]
;  ]

  ask patches [
  ; cap cell yield at 15
    set currentScore currentYield + receivedNCHBonus
    if currentScore > 15 [
;      let minus min (list receivedNCHBonus (currentScore - 15) )
      set receivedNCHBonus receivedNCHBonus - (currentScore - 15)
      set currentScore 15
    ]
    set currentScore currentScore + currentSubsidy - currentSpending
;    sxhow (word "receivedNCHBonus " receivedNCHBonus)
;    sxhow (word "currentScore " currentScore)

;    set currentProvidedValue currentProvidedValue + currentScore + providedNCHBonus
    set currentProvidedValueSelf currentProvidedValueSelf + currentScore
;    sxhow (word "currentProvidedValue " currentProvidedValue)
...
;    set totalNetProduction totalNetProduction + currentYield + receivedNCHBonus - currentSpending
;    sxhow (word "choice " choice ", currentScore " currentScore ", currentSubsidy " currentSubsidy ", totalNetProduction " totalNetProduction)
;    set totalFarmerScore totalFarmerScore + currentScore

;    let cappedYield min (list maxYield (currentYield + receivedNCHBonus - currentSpending))
;    set receivedNCHBonus cappedYield - (currentYield - currentSpending)
;    set currentScore cappedYield
    ; later on add var for full score including subsidy vs just net production
  ]
  ask farmers [
...
;  sxhow (word "totalNetProduction " totalNetProduction ", totalFarmerScore " totalFarmerScore)

;  file-print (word "Round " currentRound " Scoring Summary:")
;  file-print ""
;
;  foreach playerPosition [ ?1 ->
;    let tempScore (sum [currentScore] of patches with [owner = ?1])
;    let tempYields (sum [currentYield] of patches with [owner = ?1])
;    let tempBonuses (sum [receivedNCHBonus] of patches with [owner = ?1])
;    let tempSpending (sum [currentSpending] of patches with [owner = ?1])
...
;    set playerCurrentSpending replace-item (?1 - 1) playerCurrentSpending tempSpending
;    set playerScores replace-item (?1 - 1) playerScores (item (?1 - 1) playerScores + tempScore)
;    file-print (word "Player " ?1 ": Yields " tempYields ", Bonuses " tempBonuses ", Spending " tempSpending ", Round Score " tempScore ", Total Score " (item (?1 - 1) playerScores))
;  ]
;  file-print ""
end
...
;    sxhow (word "temp1 " temp1 " " sum temp1 " " (- tau * ln sum temp1))
    report (sum temp1)
  ]
  [ report -9999 ]
end
